	...
	<script>
		function getTree() {
			...
			var tree = [
				...
				{
					text: '{{$category->categoryName}} <span>({{$category->products->count()}})</span>',
					href:"{{url('/productsByCat/'.$category->id)}}",
					nodes: [
						@if (count($category->childs))
						@foreach($category->childs as $subcategory)
						{
							text: '{{$subcategory->categoryName}} <span>({{$category->products->count()}})</span>',
							href:"{{url('/productsByCat/'.$subcategory->id)}}",
						},
						@endforeach
						@endif
					]
