					...
					<div class="col-lg-3 col-md-4">
						<div class="sidebar ">
							<div class="widget widget-categories">
								...
								<div class="cat-list style1 widget-content" id="tree">
							...
							<div class="widget widget-countries">
								...
								<div class="widget-content">
									<form action="#" method="get" accept-charset="utf-8">
										<input type="text" name="countries" id="countries" placeholder="Category Search">
									</form>
									<ul class="box-checkbox scroll">
										@foreach($categories as $category)
											<li class="check-box country">
												<input type="checkbox" id="category{{ $category->id }}" name="category{{ $category->id }}" value=".c{{ $category->id }}">
												<label for="category{{ $category->id }}">{{ $category->categoryName }} <span>({{ $category->products->count() }})</span></label>
											</li>
										@endforeach
									</ul>
								</div>
	...
	<script>
		function getTree() {
			...
			var tree = [
				...
				{
					text: '{{$country->name}} <span>({{$country->products->count()}})</span>',
					href:"{{url('/productsByCountry/'.$country->id)}}",
				},
				@endforeach
			];
			return tree;
