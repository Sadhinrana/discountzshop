						...
						<div class="brands-list grid">
						  @foreach($brands as $brand)
							<div class="brands-item ipsotope {{strtolower(substr($brand->brandName, 0, 1))}}">
								<a href="{{URL::to('/productsByBrand/'.$brand->id)}}" class="box-cat" title="">
									<span class="menu-img"><img src="{{asset('storage/images/brands/'.$brand->brandLogo)}}" style="height: 50px;width: 75px"  alt=""></span>
									<hr />
									<div class="cat-name">
									{{$brand->brandName}}
									</div>
									<div class="numb-product">
									{{$brand->products->count()}} Product
									</div>
								</a>
